...
export const Projects = ({ content, contentClassName }) => {
  const [activeCard, setActiveCard] = useState(0);
  const ref = useRef(null);

  const { scrollYProgress } = useScroll({
    // Use `target: ref` if you want to change based on the container scroll
    // target: ref,
    container: ref,
    offset: ["0% 0%", "100% 0%"],
  });
  ...
  return (
    <motion.div
      ...
      className="h-[30rem] overflow-y-auto flex justify-center relative space-x-10 rounded-none p-10"
      ref={ref}
    >
